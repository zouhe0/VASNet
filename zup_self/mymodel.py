...
import config
_default_sdnet_cfg = {
    "MODEL": dict(config.CSC_CONFIG, ADAPTIVELAMBDA=False),
...
class DictConv2d_all(nn.Module):
    """DictConv2d for all-CSC architecture."""
...
class SDNetFusionNet_All(nn.Module):
    """Pansharpening fusion model where all conv layers use DictConv2d (CSC layers)."""
...
class SDNetFusionNet_Conv(nn.Module):
    """Same architecture as SDNetFusionNet_All, with all CSC layers replaced by ConvBlock (ablation)."""
